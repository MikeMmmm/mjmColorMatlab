function [Ra Ga RAll polyRef polyTest] = mjmCRIRaGa( SPD, cmf, wl, cmf2 )
...
% hold on; box on;
% plot(testxyzsamples(:,1),testxyzsamples(:,2),'r.')
% plot(referencexyzsamples(:,1),referencexyzsamples(:,2),'b.')
% axis equal
...
% hold on; box on;
% plot(Uref([1:8 1]),Vref([1:8 1]),'r.-')
% plot(Utestcr([1:8 1]),Vtestcr([1:8 1]),'b.-')
% axis equal
% g2Ref = polyarea(Uref([1:8]),Vref([1:8]))
% g2Test = polyarea(Utestcr([1:8]),Vtestcr([1:8]))
% G2a = 100 * g2Test ./ g2Ref;
...
% diagnostic plot
if 0
    testRGB = min(max(xyz2rgb(testXYZ/100,'Whitepoint',...
        getxyz(gettristimulus2degn(testsourcespd,range))),0),1);
    refRGB = min(max(xyz2rgb(referenceXYZ/100,'Whitepoint',...
        getxyz(gettristimulus2degn(testsourcespd,range))),0),1);
    hold on; box on;
    plot(uvreferencesamples([1:8 1],1),uvreferencesamples([1:8 1],2),'r.-')
    plot(uvc([1:8 1],1),uvc([1:8 1],2),'b.-')
    for i = 1:8
        plot(uvreferencesamples(i,1),uvreferencesamples(i,2),'.',...
            'MarkerSize',20,'Color',refRGB(i,:));
        plot(uvc(i,1),uvc(i,2),'.',...
            'MarkerSize',20,'Color',testRGB(i,:));
    end
    axis equal
end
